...
import MovieItem from "./movies/_components/movie-item";
import { filterSchema, SortOptions } from "./movies/_data/list-data";
import BTooltip from "@/components/b-tooltip";
import { signOutAction } from "@/app/(auth)/auth/_actions/auth.actions";

function MoviesPage({
  searchParams,
}: Readonly<{
  searchParams: Promise<{ page?: string; limit?: string } & Partial<Tables<"movies">>>;
}>) {
  const query = use(searchParams);
  const { page = "1", limit = "10", ...rest } = query;

  const itemsPerPage = Number.parseInt(limit) <= 50 ? Number.parseInt(limit) : 50;
  ...
  return (
    <div className="container pt-12 ">
      <div className="flex justify-between mb-5 md:mb-12">
        ...
        <button onClick={() => signOutAction()} type="button" className="flex space-x-2 items-center">
          <span>Logout</span>
          <LogOut />
        </button>
      </div>
      ...
      <List<Tables<"movies">>
        emptyNode={
          <div className="flex flex-col items-center space-y-8 ">
            ...
            <Link href={"/movies/form"} className={cn(buttonVariants({ variant: "default", className: "w-auto" }))}>
              Add a new movie
            </Link>
          </div>
        }
        skeleton={<Skeleton className="max-h-[500px] h-full w-full max-w-[300px]" />}
        loading={loading}
        className="grid grid-cols-1 items-center px-8 sm:px-0 w-full sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5"
        items={list}
        totalItems={total}
        currentPage={currentPage}
        renderItem={(movie) => (
          <Link href={`/movies/${movie.id}/view`}>
            <MovieItem className="min-w-full" movie={movie} />
          </Link>
        )}
        filterProps={{
          filterSchema,
