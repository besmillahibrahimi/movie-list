...
function MoviesPage({
  searchParams,
}: Readonly<{ searchParams: Promise<{ page?: string; limit?: string } & Partial<IMovie>> }>) {
  const query = use(searchParams);
  const { page = "1", limit = "10", ...rest } = query;

  const itemsPerPage = parseInt(limit) <= 50 ? parseInt(limit) : 50;
  ...
  const [sort, setSort] = useState<ISort<IMovie>>({ sortBy: "title", ascending: true });
